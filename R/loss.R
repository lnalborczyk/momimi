...
loss <- function (
    ...
    # max activation should not be above exec_threshold in imagined trials
    ##############################################################################
    ...
    if (uncertainty == "par_level") {
        ...
        predicted_rt_mt <- data.frame(
            ...
            # numerically finding the activation onset (RT) and offset
